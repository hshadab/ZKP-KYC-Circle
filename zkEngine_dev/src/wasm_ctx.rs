...
impl WASMArgsBuilder {
    /*───────────────────────────────────────────────────────────────*/
    /*  Helper API used by host wrappers (kyc_prover, etc.)         */
    /*───────────────────────────────────────────────────────────────*/

    /// Copy arbitrary bytes into the guest program image and return
    /// the offset where they were appended.
    pub fn copy_input_to_memory(&mut self, bytes: &[u8]) -> usize {
        let ptr = self.program.len();
        self.program.extend_from_slice(bytes);
        ptr
    }

    /// Push one `i32` CLI argument (decimal string).  Chainable.
    pub fn push_i32_arg(&mut self, v: i32) -> &mut Self {
        self.func_args.push(v.to_string());
        self
    }

    /*───────────────────────────────────────────────────────────────*/
    /*  Core builder helpers (unchanged names & signatures)         */
    /*───────────────────────────────────────────────────────────────*/

    /// Read a `.wasm` file into `program`.
    pub fn file_path(mut self, path: PathBuf) -> Result<Self, ZKWASMError> {
        self.program = std::fs::read(path)
            .map_err(|e| ZKWASMError::from(anyhow::Error::new(e)))?;
        Ok(self)
    }

    /// (Original) provide pre-built bytecode.
    pub fn bytecode(mut self, program: Vec<u8>) -> Self {
        self.program = program;
        self
    }

    /// Change the export to invoke (default `"main"`).
    pub fn invoke(mut self, name: &str) -> Self {
        self.invoke = name.to_string();
        self
    }

    /// Provide CLI-style string arguments.
    pub fn func_args(mut self, args: Vec<String>) -> Self {
        self.func_args = args;
        self
    }

    /// Optional trace-slice values.
    pub fn trace_slice(mut self, tsv: TraceSliceValues) -> Self {
        self.trace_slice_vals = Some(tsv);
        self
    }

    /// Finish and return immutable [`WASMArgs`].
